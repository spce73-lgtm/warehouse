...
// >>> افزوده شد: نسخه‌ی کش به v11 ارتقا یافت — این‌بار به‌خاطر: ۱) رفعِ ریشه‌ایِ چسبناک‌نشدنِ
// هدر ستون‌ها/فیلتر در «لیست قفسه‌ها» و «کالاهای باقیمانده» (علت واقعی: overflow-x:auto روی
// .table-wrap)؛ ۲) رفعِ ریشه‌ایِ نمایشِ تاریخِ میلادی به‌جای شمسی برای «آخرین شمارش» در نتایج
// جست‌وجو؛ ۳) رفعِ کلیک روی ردیفِ «کالاهای باقیمانده» که صفحه را عوض نمی‌کرد. بدون این تغییر
// نام کش، PWAهای از قبل نصب‌شده همچنان app.js/style.css نسخه‌ی v10 (بدون این اصلاحات) را از
// کش می‌بینند.
// >>> افزوده شد: نسخه‌ی کش به v12 ارتقا یافت — رفعِ باگِ «تعویضِ قفسه باعثِ تکرار می‌شد»:
// فرم شمارش حالا originalShelfCode هر ردیف را ردیابی می‌کند و در صورت تغییرِ قفسه، به‌جای
// افزودنِ یک اسلاتِ جدید، همان اسلاتِ قبلی را جایگزین می‌کند. بدون این تغییر نام کش، PWAهای
// از قبل نصب‌شده همچنان app.js نسخه‌ی v11 (دارای این باگ) را از کش می‌بینند.
var CACHE_NAME = 'wh-scanner-shell-v12';
